...
for aln_path in "${ALN_DIR}"/*.maple; do
    ...
    echo "Compute SPRTA for the tree ${ML_TREE_PREFIX}${aln}.treefile.nexus"
    
    echo "cd ${ALN_DIR} && ${CMAPLE_PATH} -aln ${aln} -t ${TREE_DIR}/${ML_TREE_PREFIX}${aln}.treefile.nexus -pre ${CMAPLE_SPRTA_TREE_PREFIX}${aln} ${CMAPLE_PARAMS} ${BL_FIXED_OPT} ${NOT_REROOT_OPT} ${ZERO_LENGTH_BRANCHES_OPT} ${OUT_ALT_SPR_OPT}"
    cd ${ALN_DIR} && ${CMAPLE_PATH} -aln ${aln} -t ${TREE_DIR}/${ML_TREE_PREFIX}${aln}.treefile.nexus -m ${MODEL} -pre ${CMAPLE_SPRTA_TREE_PREFIX}${aln} ${CMAPLE_PARAMS} ${BL_FIXED_OPT} ${NOT_REROOT_OPT} ${ZERO_LENGTH_BRANCHES_OPT} ${OUT_ALT_SPR_OPT}
done
